...
class TemperatureTests {
    ...
    fun testFromCentigrade() {
        ...
        val actual = Centigrade.convertTo(BigDecimal.ONE, Kelvin)
    ...
    fun testFromFahrenheit() {
        ...
        val actual = Fahrenheit.convertTo(BigDecimal.ZERO, Kelvin)
        assertEquals((expected/actual).compareTo(BigDecimal.ONE), 0)
    }

    @Test
    fun testFromRankine() {
        val expected = BigDecimal.ONE / BigDecimal("1.8")
        val actual = Rankine.convertTo(BigDecimal.ONE, Kelvin)
        assertEquals((expected/actual).compareTo(BigDecimal.ONE), 0)
    }
    ...
    fun temperatureConversion() {
        ...
        assertEquals((kelvin/Centigrade.standardize(BigDecimal("40.0"))).compareTo(BigDecimal.ONE), 0)
        assertEquals((kelvin/Fahrenheit.standardize(BigDecimal("104.0"))).compareTo(BigDecimal.ONE), 0)
